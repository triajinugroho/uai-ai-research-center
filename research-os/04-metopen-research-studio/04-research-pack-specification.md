...
- **Format/lokasi.** `docs/problem.md`, 1–2 halaman; ringkasan di Research One-Pager.
...
- **Isi minimum.** Strategi pencarian (`docs/search-strategy.md`); synthesis matrix 15–25 sumber primer dengan kolom problem, metode, data, metrik, hasil, keterbatasan, relevansi; narasi pola: konsisten, bertentangan, belum diuji; `references.bib` terverifikasi (DOI/URL).
- **Format/lokasi.** `docs/literature-map.md` (narasi + tabel tema × sumber), `literature/synthesis-matrix.csv`, `references.bib`.
...
- **Format/lokasi.** `docs/gap.md`; Issue `type:literature-gap`.
- **Template.** —
- **Gate.** G3 (dinilai), G4 (dipakai).
...
- **Format/lokasi.** `docs/research-design.md` + Research Design Card.
- **Template.** TPL-08.
...
- **Format/lokasi.** Bagian Data di `docs/research-design.md`; `data/README.md`; kartu dataset di `datasets-registry/`.
- **Template.** TPL-05.
- **Gate.** G5.
...
- **Format/lokasi.** Bagian Evaluation di `docs/research-design.md`; konfigurasi di `experiments/config/`.
- **Template.** TPL-09.
...
- **Format/lokasi.** `experiments/`, `results/`, `figures/`, `notebooks/`.
- **Template.** TPL-09.
...
- **Format/lokasi.** `docs/ethics.md`; `ETHICS.md` di root sebagai ringkasan/penunjuk.
...
- **Isi minimum.** `AI-USAGE.md`: tool, tahap riset yang dibantu, jenis bantuan (penulisan vs proses riset), cara verifikasi, apa yang ditolak; log rinci per penggunaan yang material (TPL-10): tool, tanggal, tujuan, prompt/penggunaan, output material, verifikasi, dipakai/tidak.
- **Format/lokasi.** `AI-USAGE.md` (statement) + `docs/ai-usage-log.md` atau CSV (log).
...
- **Format/lokasi.** `paper/proposal-ta.md|.tex|.pdf` sesuai format Prodi; struktur IMRaD computing di [MET-05](05-publication-backward-design.md).
...
- **Format/lokasi.** `presentation/` (slide, notulen, rekaman bila ada).
- **Template.** TPL-13.
...
Daftar minimum dari dokumen sumber dan lokasinya dalam struktur standar [TPL-15](../08-templates/15-research-repository-template.md):

```
proj-YYYY-topic/
├── README.md                  # Reproducibility README + Current Research Gate + lisensi per komponen
├── research-question.md       # (opsional) penunjuk ke docs/research-question.md untuk pembaca cepat
├── docs/
│   ├── endgame.md             # G1
│   ├── problem.md             # Problem Brief + Stakeholder/Impact (G2)
│   ├── search-strategy.md     # G3
│   ├── literature-map.md      # Literature Evidence Map (G3)
│   ├── gap.md                 # Research Gap (G3→G4)
│   ├── research-question.md   # RQ/Hypothesis + Contribution (G4)
│   ├── research-design.md     # Design, Data Plan, Baseline & Metrics, Threats v1 (G5)
│   ├── ethics.md              # Ethics & Privacy (G5)
│   └── ai-usage-log.md        # log TPL-10
├── literature/
│   └── synthesis-matrix.csv
├── references.bib
├── data/
│   └── README.md              # metadata & cara akses; data mentah sensitif TIDAK di sini
├── src/
├── notebooks/
├── experiments/
│   ├── README.md
│   └── config/
├── results/
│   └── analysis.md            # tabel hasil, error analysis, CER, Threats v2 (G7)
├── figures/
├── paper/                     # proposal TA / manuscript (G8)
├── presentation/              # pitch W8, defense W16
├── AI-USAGE.md                # AI Usage Statement
├── ETHICS.md                  # ringkasan/penunjuk ke docs/ethics.md
├── CITATION.cff · LICENSE · LICENSE-DOCS · CHANGELOG.md
```
...
[ ] 12 docs/ethics.md & ETHICS.md terisi; consent/izin bila human subjects
[ ] 13 AI-USAGE.md + log lengkap; tidak ada referensi/hasil buatan AI
...
[ ] 16 Slide & notulen W8 dan W16 tersimpan di presentation/
...
| v0.8 Manuscript Draft | + 15 draft, 13 |
| **v1.0 Research Pack** | 1–16 lengkap sesuai §5 kolom Metopen |
