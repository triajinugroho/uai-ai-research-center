...
| AI Usage Log W10 Stage `Experiment`/`Coding`: diagnosis error, kode plotting, usulan sanity check, penjelasan instruksi README — dan keputusan manusia atas tiap output; jurnal W10: **apa yang mengejutkan dari hasil pilot** | `docs/ai-usage-log.md`, ringkasan di `docs/AI-USAGE.md` ([TPL-10](../../research-os/08-templates/10-ai-usage-log-template.md)); `docs/journal/w10.md` | commit | OPS-095 |
...
- [ ] `docs/ai-usage-log.md` memuat entri Stage `Experiment`/`Coding` untuk setiap penggunaan material minggu ini; tiap anggota sudah memverifikasi entrinya; `docs/AI-USAGE.md` diperbarui; `docs/journal/w10.md` ditulis.
